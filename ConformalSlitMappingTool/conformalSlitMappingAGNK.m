...
for idx = 1:length(freeboundsList)
    [t_et, t_etp, t_etpp] = splineReparamization(vFlatten(freeboundsList{idx}, :), n, cornerIdx{idx});
    if idx == longestBoundIdx
        et = [t_et(:,1) + sqrt(-1) * t_et(:,2);et];
        etp = [t_etp(:,1) + sqrt(-1) * t_etp(:,2);etp];
        etpp = [t_etpp(:,1) + sqrt(-1) * t_etpp(:,2);etp];
    else
        et = [et; t_et(:,1) + sqrt(-1) * t_et(:,2)];
        etp = [etp; t_etp(:,1) + sqrt(-1) * t_etp(:,2)];
        etpp = [etpp; t_etpp(:,1) + sqrt(-1) * t_etpp(:,2)];
    end
end
...
if 1 == isnan(alpha)
    gam = -real(exp(i*(pi/2-thet)).*clog(1./et-1/alpha));
else
...
%
[mun,h]=fbie(et,etp,A,gam,n,5,[],1e-13,10000);
